  ...
  async _updateBlockInfo () {
    try {
      ...
      await Promise.allSettled([
        // Write latest block in aggregate.
        this.put('latest-block', {
        ...
        this.put('height', Number(blockInfo.height)),
        this.put('timestamp', Number(blockInfo.timestamp)),
        this.put('transactionCount', blockInfo.tx_count),
        this.put('size', size),
        this.put('weight', weight),
        this.put('difficulty', difficulty),
        this.put('hash', hash),
        this.put('merkleRoot', blockInfo.merkle_root)
      ])
    } catch (err) {
      console.error(err)
    }
